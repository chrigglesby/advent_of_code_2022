...
def to_cardinal(x: str):
    ...
    if x == 'L':
        return 'W'
    raise ValueError('Invalid direction supplied: ', x)
...
def a_to_b_cardinal_direction(ax: int, ay: int, bx: int, by: int) -> str:
    ...
    # Same horizontal
    if ay == by:
        return 'E' if ax < bx else 'W'

    # Same vertical
    if ax == bx:
        return 'S' if ay < by else 'N'

    # Left
    if ax < bx:
        return 'SE' if ay < by else 'NE'
...
def test_coords_to_cardinals():
    ...
    assert a_to_b_cardinal_direction(1, 1, 1, 0) == 'N'
    assert a_to_b_cardinal_direction(1, 1, 2, 0) == 'NE'
    assert a_to_b_cardinal_direction(1, 1, 2, 1) == 'E'
    assert a_to_b_cardinal_direction(1, 1, 2, 2) == 'SE'
    assert a_to_b_cardinal_direction(1, 1, 1, 2) == 'S'
    assert a_to_b_cardinal_direction(1, 1, 0, 2) == 'SW'
    assert a_to_b_cardinal_direction(1, 1, 0, 1) == 'W'
    assert a_to_b_cardinal_direction(1, 1, 0, 0) == 'NW'
