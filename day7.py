...
def parse(input: str):
    ...
    def is_file():
        # We handle all other valid input so this can be assumed True
        return True

    # Actions
    def set_current_dir(s: str):
        global dir_depth
        dest = s[5:]
        ...
        if dest == '..':
            ...
            dir_depth.append(s[5:])

        # Consideration: There is potentially a case where cd is used to
        # navigate to a directory  without it being ls'd; this would lead
        # to a case where we don't create a record of that directory -
        # for now, assuming this won't happen

        # print(f'set cd {current_dir()}')

    def create_dir(s: str):
        global dir_id_inc
        global structure
        ...
        # print(f'create dir {directory.name}')

    def create_file(s: str):
        global file_id_inc
        global structure
        ...
        # print(f'create file {file.name}')

    for line in output:
        if not is_ls(line):  # Currently nothing to do for ls

            if is_cd(line):
                set_current_dir(line)
...
def get_dir(name: str):
    ...
    for s in structure:
        if isinstance(s, Dir) and s.name == name:
            return s
...
def sum_dir_file_sizes(dir_sizes: dict):
    for d in dir_sizes:
        dir_sizes[d] = sum(dir_sizes[d])
...
def sum_dir_sizes_with_sub_directories(dir_sizes: dict):
    ...
    def sort_by_depth(item):
        ...
        if y:
            return y.depth
        return 0  # This is root

    # We must sort by depth to ensure when adding directories together we
    # don't do this BEFORE sub directories are added up
    dir_depth_order_references = sorted(dir_sizes, key=sort_by_depth, reverse=True)

    for d in dir_depth_order_references:
        directory = get_dir(d)
...
# Updates structure with depth in directory tree
def assign_structure_depth():
    # Loop dirs
    # For dir, follow parent and increment up
    # Until root, that = final depth
    # Root = 0
    global structure

    for d in structure:
        if isinstance(d, Dir) and not d.name == '/':  # Skip root as it's already depth 0
            depth = 0
            parent = None
            target = d
